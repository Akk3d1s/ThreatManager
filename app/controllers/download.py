...
def download_threat_file(threat_id=None):
    ...
    try:
        ...
        if len(files) == 1:
            ...
            return send_file(file_path+files[0].file,
                             attachment_filename=files[0].file,
                             as_attachment=True)
        zip_file_name = 'threat'+str(threat_id)+'.zip'
        Logger.success(request.path)
        return send_file(file_path+zip_file_name,
...
def download_all_cases():
    ...
    try:
        ...
        if current_user.role_id == 1:
            threats = db.session\
                .query(Threat, ThreatStatus, ThreatCategory, User, UserRole)\
                .join(ThreatStatus)\
                .join(ThreatCategory)\
                .join(User)\
                .join(UserRole)\
                .filter(UserRole.id==current_user.role_id)\
                .group_by(Threat)\
                .all()
        else:
            threats = db.session\
        ...
        output.headers["Content-Disposition"] = f"attachment; filename=threats_{current_date}.csv"
