...
def index():
    ...
    # police roles
        ...
        threats = db.session.query(User, Threat, ThreatStatus, ThreatCategory).filter(User.id==Threat.user_id).filter(Threat.status_id==ThreatStatus.id).filter(Threat.category_id==ThreatCategory.id).all()
        comments = db.session.query(User, Comment).filter(User.id==Comment.user_id).all()
        return render_template("editor.html", title='Home Page', threats=threats, comments=comments, form=form)
    else:
