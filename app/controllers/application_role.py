...
def role_application(role_id=None):
    ...
    try:
        ...
        r_a = RoleApplication(user_id=current_user.id, role_id=role_id)
        r_a.save()
...
def role_application_list():
    """Used by the Admin role to send to page where view permission
    changes from other uses are displayed"""
    try:
        if not Authenticator.role_access_check(request.path):
            return redirect(url_for('index'))
        role_applications = db.session\
            ...
            .filter(UserRole.id == RoleApplication.role_id).all()
        Logger.success(request.path)
...
def approve_role_application(role_application_id=None):
    ...
    try:
        ...
        if not IdValidator.validate_role_application_id(role_application_id):
            return redirect(url_for('index'))
        r_a = RoleApplication.query.filter(RoleApplication.id == role_application_id).first()
        user = User.query.filter(User.id == r_a.user_id).first()
        user.role_id = r_a.role_id
