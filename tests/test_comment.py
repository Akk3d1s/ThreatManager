...
def test_successful_comment_without_file(client):
    ...
    with client.session_transaction() as session:
        session['_user_id'] = '1'
    data = dict(comment='comment_test1', file=(io.BytesIO(),''))
    rv = comment(client, data)
    assert b'comment_test1' in rv.data
...
def test_successful_comment_single_file(client):
    ...
    rv = comment(client, data)
    assert b'comment_test_single_file' in rv.data
...
def test_successful_comment_multiple_file(client):
    ...
    rv = comment(client, data)
    assert b'comment_test_multiple_file' in rv.data
...
def test_failed_comment_unauthorized_role(client):
    ...
    rv = comment(client, data)
    assert b'comment_test_unauthorized_role' not in rv.data
...
def test_failed_comment_unrelated_citizen(client):
    ...
    rv = comment(client, data)
    assert b'comment_test_unrelated_citizen' not in rv.data
...
def test_failed_incorrect_file_extension(client):
    ...
    rv = comment(client, data)
    assert b'comment_test_incorrect_extension' not in rv.data
...
def test_failed_incorrect_oversize_files(client):
    ...
    rv = comment(client, data)
    assert b'comment_test6_oversize_files' not in rv.data
...
def test_failed_incorrect_exceed_file_amount(client):
    ...
    rv = comment(client, data)
    assert b'comment_test_exceed_file_amount' not in rv.data
